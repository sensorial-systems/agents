...
use crate::{Communicator, Content, Conversation};
...
impl Communicator for Agent {
    ...
    async fn send(&mut self, _recipient: &mut dyn Communicator, _conversation: &mut Conversation, _content: Content) {
    }

    async fn receive(&mut self, _sender: &mut dyn Communicator, _conversation: &mut Conversation) {
    }
}
